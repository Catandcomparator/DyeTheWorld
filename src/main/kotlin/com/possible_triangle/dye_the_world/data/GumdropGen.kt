...
fun <T : Block, P> BlockBuilder<T, P>.gumdropBlockstate() = blockstate { context, provider ->
    val models = listOf(true, false).associateWith { powered ->
        val suffix = if (powered) "_pressed" else ""
        provider.models().getBuilder(context.name + suffix)
...
fun <T : Item, P> ItemBuilder<T, P>.gumdropModel() = model { context, provider ->
    provider.withExistingParent(context.name, SNOWY_SPIRIT.createId("item/gumdrop"))
        .texture("texture", Constants.MOD_ID.createId("block/$SNOWY_SPIRIT/gumdrop/$dye"))
}

fun <T : Item, P> ItemBuilder<T, P>.gumdropRecipe() = recipe { context, provider ->
